    ...
    <main className="relative min-h-dvh">
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-5 py-8">
        ...
        <div className="mt-6 flex items-center gap-2 font-display text-2xl font-bold">
          <Trophy className="h-6 w-6 text-[var(--cyan)]" /> Leaderboard
        </div>
        <p className="mt-1 text-sm text-muted-foreground">Top 100 ranked players by Elo. Guests aren't ranked — <Link to="/auth" className="underline hover:text-foreground">create an account</Link> to compete.</p>
        ...
        <div className="mt-6 glass rounded-2xl overflow-hidden">
          {rows === null ? (
            ...
            <div className="py-12 text-center text-sm text-muted-foreground">No ranked players yet. Be the first!</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="text-[10px] uppercase tracking-widest text-muted-foreground">
                <tr className="border-b border-white/5">
                  ...
                  <th className="text-right py-2 px-3 hidden sm:table-cell">W / L</th>
                  <th className="text-right py-2 px-3 hidden sm:table-cell">Games</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.user_id} className={`border-b border-white/5 last:border-0 ${r.user_id === userId ? "bg-[var(--violet)]/10" : ""}`}>
                    ...
                    <td className="py-3 px-3 font-display">{r.username}{r.user_id === userId && <span className="ml-2 text-[10px] text-[var(--cyan)]">YOU</span>}</td>
